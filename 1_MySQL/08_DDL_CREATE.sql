...
/*
	PRIMARY KEY (기본키)
    - 테이블에서 각 행동을 식별하기 위해 사용될 컬럼에 부여하는 제약조건 (식별자 역할)
...
-- mem_grade 테이블에서 grade_code가 2인 등급삭제
DELETE FROM mem_grade
WHERE grade_code = 2;
-- >> 자식 테이블(member)에 2라는 값을 사용하고 있기 때문에 삭제X
-- mem_grade 테이블에서 grade_code가 2인 등급삭제
DELETE FROM mem_grade
WHERE grade_code = 3;
-- >> 자식 테이블(member)에 3이라는 값을 사용하지 않기 떄문에 삭제 O

SELECT * FROM member; 
DELETE FROM member WHERE mem_no = 4;

/*
		자식테이블 생성시 외래키 제약조건을 부여할 떄 삭제옵션 지정 가능
...
-- 모든 행에 대해서 매번 false이기 떄문에 테이블 구조만 복사!

-- employee 테이블에서 사번, 직원명, 급여, 연봉만 저장
-- employee_copy 테이블로 생성
DROP TABLE employee_copy;
CREATE TABLE employee_copy
SELECT * FROM kh.employee;
SELECT * FROM kh.employee;
