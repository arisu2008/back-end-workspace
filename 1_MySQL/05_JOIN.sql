...
SELECT * FROM employee; -- emp_id, emp_name, job__code, dept_code, salary
...
SELECT * FROM job; -- job_cod, job_name
SELECT * FROM department; -- dept_id, dept_title
...
FROM employee
    ...
    JOIN department ON (dept_code = dept_id)
-- WHERE emp_no LIKE '7%';
-- WHERE substr(emp_no) IN ( 7 )
WHERE emp_no LIKE '7____-2%'
	AND emp_name LIKE '전%';
-- 3. 보너스를 받은 직원들의 직원명, 보너스, 연봉, 부서명, 근무지역 조회 
SELECT * FROM employee; -- emp_name, bonus, sakart, dept_code
SELECT * FROM department; -- dept_id, dept_title, location_id
SELECT * FROM location; -- local_code, local_name

SELECT
 emp_name, bonus, (salary + salary * bonus)*12, dept_title, local_name
FROM employee
	JOIN department ON (dept_code = dept_id)
	JOIN location ON (location_id = local_code)
WHERE bonus is not null;
...
SELECT * FROM national; -- national_code, national_naem
...
SELECT * FROM employee; -- dept_code, sa;aru
SELECT * FROM department; -- dept_id, dept_title
...
SELECT * FROM job; -- job_codm, job_name
...
FROM employee
	JOIN department ON (dept_code = dept_id)
    JOIN job USING (job_code)
    JOIN national ON (national_id = local_code)
    JOIN sal_grade ON (salary BETWEEN min_sal AND max_sal);
