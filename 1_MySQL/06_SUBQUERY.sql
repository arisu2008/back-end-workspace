/*
    ...
    1. 당닝ㄹ행 서브쿼리(SINGLE ROW SUBQUERY)
...
SELECT emp_id, emp_name, dept_code, format(avg(salary), 0)
FROM employee
GROUP BY emp_id
HAVING 3047662.6087 < avg(salary);
...
SELECT dept_code
FROM employee
WHERE emp_name = '전지연';(SELECT emp_id, emp_name, phone, job_name, dept_title, hire_date
					FROM employee
                    JOIN job USING (job_code)
					JOIN department ON (dept_code = dept_id)
					WHERE emp_name = '전지연'
...
FROM employee
GROUP BY dept_code
HAVING sum(salary) = (SELECT max(sun_sal)
						FROM (SELECT dept_code, sum(salary) sum_sal
								FROM employee
...
/*
	2. 다중행 서브쿼리
    - 서브퀴리의 조회 결과 값의 개수가 여러 행 일 떄 (여러행 한열)
...
FROM employee
GROUP BY dept_code;
-- 2890000, 3660000, 2490000, 3760000, 3900000, 2550000, 8000000
...
if(manager_id is not null , '사원')
FROM employee;

SELECT manager_id
FROM employee
WHERE manager_id IS NOT NULL; -- >> 힌트 SELECT 문 내에 조건으로 사용!
...
*/
-- 대리 직급임에도 과장 직급들의 최소 급여보다 많이 받는 직원의 사번, 이름, 직급, 급여 조횝
-- 1) 과장 직급들의 급여
SELECT emp_id, emp_name, dept_code, salary
FROM employee
	JOIN job USING(job_code)
...
FROM employee
WHERE emp_name = '박나라'; -- D5 , j7

SELECT emp_name, emp_id, manager_id, job_code
FROM employee
 WHERE (dept_code, job_code) = ('D5', 'j7');

/*
	4.다중행 다중열 서브쿼리
    - 서브쿼리의 조회 결과값이 여러 행, 여러 열일 경우
*/
-- 각 직급별로 최소 급여를 받는 사원들의 서번, 이름, 직급코드, 급여 조회
-- 각 직급별로 최소 급여

SELECT dept_code, min(salary)
FROM employee
GROUP BY dept_code; 
-- 2320000, 1380000, 1550000, 1800000, 2800000, 2000000, 3700000
